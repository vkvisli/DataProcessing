...
from washing machines associated with the CoSSMic project. The manual classification is done with 2/3
of the cycles (training) and automatic classification with 1/3 (verification).
JSON files for each cluster is saved and visual charts showing the clusters are displayed.
...
data_location = "./data/edited/wm_runs/"
cluster_output_location = "./data/edited/wm_clusters/"
filenames = ["Washing Machine 1.csv","Washing Machine 2.csv","Washing Machine 3.csv",
             "Washing Machine 4.csv","Washing Machine 5.csv","Washing Machine 6.csv"]
appliance_list = []

# Set which time resolution the washing machine data is read with
minute_res = 1
...
def plot_machine_separated_runs(runs, title=""):
    """
    Plots the given machine separated runs, with a unique color for each washing machine

    :param runs: (list) A list containing the washing machine runs
    ...
    plt.savefig("./figures/appliances/clustering/WM1-6_clustering_machine_separated.png")
...
def plot_cluster_rectangles(runs, clusters, title=""):
    ...
    # Create directory 'figures', 'appliances', and 'clustering' if they do not exist, and save figures
    os.makedirs("./figures/appliances/clustering/", exist_ok=True)
    plt.savefig("./figures/appliances/clustering/WM1-6_clustering_rectangles.png")
...
def plot_classification_results(runs, clusters=None, not_classified_runs=None, title=""):
    ...
    :param runs: (list) A list containing the washing machine runs
    :param clusters: (list) A list of Cluster objects
    :param not_classified_runs: (list) A list of the washing machine runs that have not been successfully classified
    :param title: (str) The title of the chart
    """
    plt.clf()
    plt.figure(figsize=(14, 8.75))
    ...
    # Create directory 'figures', 'appliances', and 'clustering' if they do not exist, and save figures
    os.makedirs("./figures/appliances/clustering/", exist_ok=True)
    plt.savefig("./figures/appliances/clustering/WM1-6_clustering_classification_results.png")
...
def plot_final_clusters(clusters, not_classified_runs, title):
    ...
    # Create directory 'figures', 'appliances', and 'clustering' if they do not exist, and save figures
    os.makedirs("./figures/appliances/clustering/", exist_ok=True)
    plt.savefig("./figures/appliances/clustering/WM1-6_clustering_final_clusters.png")
...
def save_clusters_json(clusters, location):
    ...
    cluster_nr = 1
    for c in clusters:
        filename = "WM cluster " + str(cluster_nr) + " (" + str(c.min_duration) + "-" + str(c.max_duration) + " min" \
...
# For each washing machine, read single run data (the result from time-series splitting) and create an Appliance object
...
# Retrieve all washing machine runs
all_runs = []
...
clusters = ClusterParameters.WM_clusters()
...
plot_machine_separated_runs(all_runs_machine_separated, "Washing machines 1-6\nDuration and consumption of runs")
plot_cluster_rectangles(training, clusters, title="Washing machines 1-6\nClustering of training runs")
plot_classification_results(training, clusters, not_classified_runs, "Washing machines 1-6\nClassification of test runs")
plot_final_clusters(clusters, not_classified_runs, "Clusters for washing machine runs")
